...
Required env vars (post-import mode):
  ...
  MAISA_AUTH_CREDENTIAL   JSON secret: {"dev": "<cookie>", "pre": "<cookie>", ...}
...
def main() -> None:
    ...
    if not target_url or not raw_credential:
        print("OK")
        return

    environment = os.environ.get("MAISA_ENVIRONMENT", "").strip()
    auth_credential = resolve_credential(raw_credential, environment) if environment else raw_credential
